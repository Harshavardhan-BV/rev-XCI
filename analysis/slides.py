...
def hmap(fns,axs,lbl,inp,txt,titl=''):
    ...
    plt.title(titl)
    plt.tight_layout()
    figname='../writing/slides/figures/'+txt+'-'+inp+'-sse-hmap.pdf'
    plt.savefig(figname)
    plt.close(fig)
    fig=plt.figure(figsize=(siz+2,siz))
    ...
    plt.title(titl)
    plt.tight_layout()
    figname='../writing/slides/figures/'+txt+'-'+inp+'-rsq-hmap.pdf'
    plt.savefig(figname)
    plt.close(fig)
...
def timeseries_topo(f,inp,titl=''):
    iname='../input/'+inp+'.csv'
    idf=pd.read_csv(iname)
    figname='../writing/slides/figures/'+f+'-'+inp+'-timeseries.pdf'
    # Solver input parameters
    t=idf['t'].values
    trang=(t[0],t[-1])
...
def parmcomp(f,ts=False):
    if ts:
        ...
        ts=''
    figname='../writing/slides/figures/'+f+ts+'-parmcomp.pdf'
...
def timeseries_noise_violin(f,inp):
    ...
    figname='../writing/slides/figures/'+f+'-'+inp+'-noise-violin.pdf'
...
timeseries_topo('IIII', 'iPSC_timeshifted')
timeseries_topo('IIAA', 'iPSC_timeshifted')
timeseries_topo('IINN', 'iPSC_timeshifted')
timeseries_topo('IIII', 'Partial_timeshifted')
...
hmap(combi2d,axs,lbl,'iPSC_timeshifted','vary_cross-AA')
hmap(combi2d,axs,lbl,'Partial_timeshifted','vary_cross-AA')

combi2d=np.array([['AAII','AIII'],['IAII','IIII']])
hmap(combi2d,axs,lbl,'iPSC_timeshifted','vary_cross-II')
hmap(combi2d,axs,lbl,'Partial_timeshifted','vary_cross-II')

lbl=['Self-connection of $X_a$','Self-connection of $X_i$']
combi2d=np.array([['IIAA','IIAI','IIAN'],['IIIA','IIII','IIIN'],['IINA','IINI','IINN']])
axs=['A','I','N']
hmap(combi2d,axs,lbl,'iPSC_timeshifted','vary_self')
hmap(combi2d,axs,lbl,'Partial_timeshifted','vary_self')
